...
from model.EffMambaHSI import EffMambaHSI
...
def get_parser():
    ...
    parser.add_argument('--data_set_path',type=str,default='/data/ty22/data/data')
...
if __name__ == '__main__':
    ...
    for exp_idx,curr_seed in enumerate(seed_list):
        ...
        net = EffMambaHSI(in_channels=channels, num_classes=class_count, hidden_dim=128)
        logger.info(paras_dict)
        logger.info(net)

        x = transform(np.array(img))
        # device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu") #003
        ...
        best_net = EffMambaHSI(in_channels=channels, num_classes=class_count, hidden_dim=128)
    ...
    logger.info(
        f'Acc per class={np.round(np.mean(EACH_ACC_ALL, 0) * 100, decimals=2)}+-{np.round(np.std(EACH_ACC_ALL, 0) * 100, decimals=2)}'
          )

    logger.info(
        f"Average training time={round(np.mean(Train_Time_ALL), 2)}+-{round(np.std(Train_Time_ALL), 3)}"
        )
    logger.info(
        f"Average testing time={round(np.mean(Test_Time_ALL) * 1000, 2)}+-{round(np.std(Test_Time_ALL) * 1000, 3)}"
          )
